...
HvV::IndvidualChallenge(heroes,villains,number_survivors_per_tribe);
	...
	if (days>20)
	{
		...
		if (remaining_survivors <= 3)
		{
			cout << "Only 3 survivors left in the combined array." << endl;
			for (int i = 0; i < number_survivors_per_tribe; i++)
			{
			...
			do {
				cin >> user_vote;
			} while (combined[user_vote]->getEliminated() == 1);
			user_vote = user_vote - 10;
		...
		for (int i = 0; i < number_survivors_per_tribe; i++)
		{
			...
			if (combined[i]->getEliminated() == 0 && combined[i]->getPlayerCharacter() == 0)
			{
				...
				if (combined[i]->getAllied() != 2)
				{
					...
					do
					{
						cpus_vote = rand() % numCombined;
					} while (cpus_vote == i && combined[cpus_vote]->getEliminated() == 1 && (combined[i]->getAllied() == 1 && i == choice - 10));
		}
