...
public:
Survivor(string first_name, string tribe, int strength, int vote_ID,int player, int votes_against, int elim, int allied,int sit_out,int tricks, int trust,int enemies, int immune,int tribe_immune, int challenge_wins, int allies_made,int trust_made) 
{
...
{
public: 
Samoa(string first_name, string tribe, int strength, int vote_ID, int player, int votes_against, int elim, int allied,int sit_out, int tricks ,int trust, int immune,int enemies, int tribe_immune, int challenge_wins, int allies_made,int trust_made): Survivor (first_name, tribe, strength, vote_ID,player,  votes_against, elim, allied, sit_out,tricks, trust, immune, enemies, tribe_immune, challenge_wins, allies_made, trust_made)
{
}
...
class HvV: public Survivor
{
	...
	int Idol;
	public:
	HvV(string first_name, string tribe, int strength, int vote_ID, int player, int votes_against, int elim, int allied, int sit_out,int tricks, int trust, int immune,int enemies ,int tribe_immune, int challenge_wins, int allies_made,int trust_made, int Idol): Survivor (first_name, tribe, strength, vote_ID,player,votes_against, elim, allied, sit_out, tricks, trust, immune,enemies, tribe_immune, challenge_wins, allies_made, trust_made)
		{
			this->Idol=Idol;
		}
...
if(choice>9)
{
	...
	villains[user_vote]->setVotesAgainst(1);
		...
		if(villains[i]->getAllied()==1)
			{
			int blindside_chance;
			int blindside_result=0; 
			for (int i=0; i<number_survivor_per_tribe; i++)
				{
				if(villains[choice-10]->getAlliedMade()>3)
				{
					if(villains[i]->getAllied()==1)
					{
					...
					int newTrust=villains[i]->getTrust();
					 blindside_chance=NewTricks-newTrust;
					 blindside_result= rand() % blindside_chance;
					 cout << blindside_result << endl;
					}
				}
			}
				//TRIBE MATES TURN ON YOU
				if(blindside_result>80)
				{
					cout <<"BlindSide" << endl;
					cpu_vote =choice-10;
				}
...
void random_event_villian(HvV* villian[], int choice)
{
	int random_Number_event = rand() % 10 + 1;
	int upgrade1 = villian[choice]->getStrength() - 1;
	int upgrade2 = villian[choice]->getStrength() + 1;
			...
			case 2:
				cout << "Whoops! You've tripped and fell. Unfortunately, you twisted your ankle and have suffered the consequences." << endl;
				cout << "-1 Strength" << endl;
				villian[choice]->setStrength(upgrade1);
				break;
			case 3:
				cout << "Night time roles around..." << endl;
				cout << "You wake up in the middle of the night. Someone is snoring extremely loud. You are frustrated and have trouble falling back asleep." << endl;
			...
			case 5:
				...
				cout << "+1 Strength" << endl;
				villian[choice]->setStrength(upgrade2);
				break;
			case 6:
				cout << "You found a little bit of extra food in the forest." << endl;
				cout << "+1 Strength" << endl;
...
switch(season)
	{
        ...
        new HvV("#0 Sugar Kiper",  "Heroes",             13,        0, 0,0,        0,0,0,    30,    0,   0,        0, 0,           0,                  0,      0,        0),
        new HvV("#1 Stephenie LaGrossa",  "Heroes",      15,        1, 0, 0,      0, 0, 0,  45,    0,   0,        0,   0,         0,                  0,      0,        0),
        new HvV("#2 Cercie Fields",  "Heroes",           11,        2,  0, 0,     0, 0, 0,  50,    0,   0,        0 ,    0,       0,                  0,      0,        0),
        new HvV("#3 Tom Westman",  "Heroes",             16,        3, 0,   0,    0, 0, 0,  40,    0,   0,        0,       0,     0,                  0,      0,        0),
        new HvV("#4 James Clement",  "Heroes",           15,        4,  0,   0,   0, 0, 0,  40,    0,   0,        0,  0,          0,                  0,      0,        0),
        new HvV("#5 J.T Thomas",  "Heroes",              16,        5,  0, 0,     0, 0, 0,  20,    0,   0,        0,   0,         0,                  0,      0,        0),
        new HvV("#6 Amanda Kimmel",  "Heroes",           15,        6,  0, 0,     0, 0, 0,  29,    0,   0,        0,    0,        0,                  0,      0,        0),
        new HvV("#7 Candice Woodcock",  "Heroes",        14,        7,  0, 0,     0, 0, 0,  50,    0,   0,        0,     0,       0,                  0,      0,        0),
        new HvV("#8 Rupert Boneham",  "Heroes",          17,        8,  0, 0,     0, 0, 0,  20,    0,   0,        0,      0,      0,                  0,      0,        0),
        new HvV("#9 Colby Donaldson",  "Heroes",         15,        9,  0, 0,     0,  0, 0, 18,    0,   0,        0,       0,     0,                  0,      0,        0)
				};
    ...
    //The ID's need to be different from the ones for the heros, this will be important during merge phase 
				...
				{
							//name           tribe    strength, vote_id, elim, allied, trust, immune, enemies, tribe_I, challenge wins, allies made, trust built, Idol
        new HvV("#10 Courtney Yates",  "Villains",    12,         10, 0,  0,   0,0, 0,    30,     0,   0,        0,      0,  0,              6,                0,         0),
        new HvV("#11 Russell Hantz",  "Villains",     15,         11, 0,   0,  0, 1, 0,   90,     0,   0,        0,      0,    0,            0,                0,         0),
        new HvV("#12 Randy Bailey",  "Villains",      13,         12, 0,   0,  0, 1, 0,   43,     0,   0,        0,      0,      0,          0,                0,         0),
        new HvV("#13 Tyson Apostol",  "Villains",     17,         13, 0,  0,   0, 1, 0,   50,     0,   0,        0,      0,   0,             0,                0,         0),
        new HvV("#14 Boston Rob",  "Villains",        18,         14, 0,  0,   0, 1, 0,   55,     0,   0,        0,      0,   0,             0,                0,         0),
        new HvV("#15 Coach Wade",  "Villains",        14,         15, 0,  0,   0, 1, 0,   30,     0,   0,        0,      0,    0,            0,                0,         0),
        new HvV("#16 Jerri Manthey",  "Villains",     15,         16, 0,  0,   0, 1, 0,   30,     0,   0,        0,      0,     0,           0,                0,         0),
        ...
        new HvV("#19 Danielle DiLorenzo", "Villains", 16,         19, 0,  0,   0, 0, 0,   60,     0,   0,        0,      0,     0,           0,                0,         0)
				...
				if(character_choice == 1)
					...
					if (RNE>50)
					{
						random_event_hero(heroes, choice);
					}
					}
				if (character_choice == 2)
					...
					if (RNE > 50)
					{
						random_event_villian(villains, choice);
					}
				}
	//TRIBAL STAGE 
    while(days<20)
